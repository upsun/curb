...
func probeNetworkForwarding() {
	...
	cmd := exec.Command(curbBin, "--tun", "--domains", "localhost",
		"--write", "*", "--exec", "*", "--",
...
func TestCurb_Net_LoopbackDNSRouted(t *testing.T) {
	...
	cmd := exec.Command(curbBin, "--tun", "--write", "*", "--domains", "*", "--", "getent", "hosts", "example.com")
	out, err := cmd.CombinedOutput()
...
func TestCurb_Net_TCPForwarding(t *testing.T) {
	...
	cmd := exec.Command(curbBin, "--tun", "--domains", "*", "--allow-http", "--write", "*", "--exec", "*", "--",
		"sh", "-c", fmt.Sprintf("curl --noproxy '*' -s --connect-timeout 10 http://%s/ -H 'Host: example.com' | head -c 200", ip))
	out, err := cmd.CombinedOutput()
...
// TestCurb_Net_TLSBlockedAtTUN verifies that port 443 is blocked at the TUN layer.
// HTTPS must go through the MITM proxy, not the TUN.
func TestCurb_Net_TLSBlockedAtTUN(t *testing.T) {
	requireNetNS(t)

	ip := resolveForTest(t, "example.com")

	cmd := exec.Command(curbBin, "--tun", "--domains", "*", "--write", "*", "--exec", "*", "-v", "--",
		"sh", "-c", fmt.Sprintf("curl --noproxy '*' -s --connect-timeout 5 --resolve example.com:443:%s https://example.com/ 2>&1", ip))
	out, _ := cmd.CombinedOutput()
	outStr := string(out)
	assert.Contains(t, outStr, "curb: tcp_connect blocked:",
		"expected port 443 block log message")
}
...
func TestCurb_Net_NoRawSocketEscape(t *testing.T) {
	...
	cmd := exec.Command(curbBin, "--tun", "--domains", "*", "--write", "*", "--exec", "*", "--",
...
func TestCurb_DNS_AllowedDomainResolves(t *testing.T) {
	...
	cmd := exec.Command(curbBin, "--tun", "--write", "*", "--domains", "example.com", "--",
...
func TestCurb_DNS_StarAllowsEverything(t *testing.T) {
	...
	cmd := exec.Command(curbBin, "--tun", "--write", "*", "--domains", "*", "--",
		"getent", "hosts", "example.com")
	out, err := cmd.CombinedOutput()
	outStr := filterCurbOutput(string(out))
...
func TestCurb_DNS_BlockedLogMessage(t *testing.T) {
	...
	cmd := exec.Command(curbBin, "--tun", "--write", "*", "-v", "--domains", "example.com", "--",
...
// domain by using its IP address directly. Port 80 is blocked by default and
// port 443 is always blocked at the TUN layer.
func TestCurb_DNS_Bypass_DirectIP(t *testing.T) {
	...
	cmd := exec.Command(curbBin, "--tun", "--write", "*", "--exec", "*", "-v",
		"--domains", "other-domain-not-example.test", "--",
		"curl", "--noproxy", "*", "-sS", "--connect-timeout", "5", fmt.Sprintf("http://%s/", ip), "-H", "Host: example.com")
	out, _ := cmd.CombinedOutput()
	outStr := string(out)
...
// --- TLS blocked at TUN layer tests ---
//
// Port 443 is unconditionally dropped at the TUN layer. HTTPS must go
// through the MITM proxy.

// TestCurb_TLS_Port443Blocked verifies that port 443 is dropped even for allowed domains.
func TestCurb_TLS_Port443Blocked(t *testing.T) {
	requireNetNS(t)
...
// TestCurb_TLS_DirectIPBlocked verifies that HTTPS by direct IP is blocked at port 443.
func TestCurb_TLS_DirectIPBlocked(t *testing.T) {
	...
	cmd := exec.Command(curbBin, "--tun", "--write", "*", "--exec", "*", "-v",
		"--domains", "other.test", "--",
		"curl", "--noproxy", "*", "-sk", "--connect-timeout", "5", fmt.Sprintf("https://%s/", ip))
	out, _ := cmd.CombinedOutput()
	outStr := string(out)
	assert.Contains(t, outStr, "curb: tcp_connect blocked:",
		"expected port 443 block log message")
...
// TestCurb_TLS_PlaintextOn443Blocked verifies that any traffic on port 443 is blocked.
func TestCurb_TLS_PlaintextOn443Blocked(t *testing.T) {
	...
	cmd := exec.Command(curbBin, "--tun", "--write", "*", "--exec", "*", "-v",
		"--domains", "example.com", "--",
		"curl", "--noproxy", "*", "-s", "--connect-timeout", "5", fmt.Sprintf("http://%s:443/", ip))
	out, _ := cmd.CombinedOutput()
	outStr := string(out)
	assert.Contains(t, outStr, "curb: tcp_connect blocked:",
		"expected port 443 block log message")
...
func TestCurb_HTTP_AllowedHostSucceeds(t *testing.T) {
	...
	cmd := exec.Command(curbBin, "--tun", "--write", "*", "--exec", "*",
...
func TestCurb_HTTP_BlockedHostGets403(t *testing.T) {
	...
	cmd := exec.Command(curbBin, "--tun", "--write", "*", "--exec", "*",
		...
		"sh", "-c", fmt.Sprintf("curl --noproxy '*' -sI --connect-timeout 5 http://%s/ -H 'Host: blocked.com' | head -1", ip))
	out, err := cmd.CombinedOutput()
	outStr := filterCurbOutput(string(out))
...
func TestCurb_Net_NonStandardPortDropped(t *testing.T) {
	...
	cmd := exec.Command(curbBin, "--tun", "--write", "*", "--exec", "*",
		"--domains", "example.com", "--",
		"curl", "--noproxy", "*", "-s", "--connect-timeout", "3", fmt.Sprintf("http://%s:8080/", ip))
	out, _ := cmd.CombinedOutput()
	outStr := filterCurbOutput(string(out))
...
// TestCurb_Net_WildcardAllowsHTTP verifies that --domains '*' with --allow-http
// allows HTTP traffic through the TUN. Port 443 is always blocked at TUN.
func TestCurb_Net_WildcardAllowsHTTP(t *testing.T) {
	requireNetNS(t)
	requireExternalHTTP(t)

	cmd := exec.Command(curbBin, "--tun", "--write", "*", "--exec", "*",
		"--domains", "*", "--allow-http", "--",
		"sh", "-c", "curl --noproxy '*' -sI --connect-timeout 10 http://example.com/ | head -1")
	out, err := cmd.CombinedOutput()
	outStr := filterCurbOutput(string(out))
	require.NoError(t, err, "expected wildcard to allow HTTP: %s", outStr)
...
`), 0o644))
	...
	// Run in default proxy mode (no --tun, no --unrestricted-net).
	cmd := exec.Command(curbBin,
		"--domains", "localhost",
		"--read", helperDir,
		"--exec", helperDir,
