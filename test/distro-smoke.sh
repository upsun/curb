...
test_distro() {
	...
	results=$(docker run "${DOCKER_FLAGS[@]}" "$image" sh -c "
		...
		# Pre-flight: --dry-run must succeed for any tests to be meaningful.
		# If it fails, the host likely blocks user namespaces (e.g. AppArmor).
		probe=\$(/curb --dry-run 2>&1)
		if [ \$? -ne 0 ]; then
			# Extract the last non-empty line as the reason.
		...
		out=\$(/curb -- sh -c 'touch \${TMPDIR:-/tmp}/test && rm \${TMPDIR:-/tmp}/test' 2>&1)
		if [ \$? -eq 0 ]; then
		...
		out=\$(/curb -- sh -c '! touch /usr/bin/test-escape' 2>&1)
		if [ \$? -eq 0 ]; then
		...
		out=\$(/curb -- cat /etc/os-release 2>&1)
		if [ \$? -eq 0 ] && [ -n \"\$out\" ]; then
		...
		# Pre-check: verify direct HTTPS works (CA bundle functional).
		# Skip proxy tests if the distro's CA bundle can't verify the target.
	...
	while IFS= read -r line; do
		case "$line" in
			RESULT:ABORT:*)
				...
				echo "         Try CURB_TEST_RELAXED=1 or see docs/troubleshooting.md"
				;;
			RESULT:PASS:*)
